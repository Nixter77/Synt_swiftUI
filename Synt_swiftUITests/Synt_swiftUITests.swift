...
struct Synt_swiftUITests {
    ...
    @Test func voiceManagerStealsUnderPolyphonyPressure() async throws {
        let manager = VoiceManager()
        for midiNote in 0..<VoiceManager.maxVoices {
            manager.addVoices(
                midiNote: midiNote,
                velocity: 1.0,
                unisonVoices: 1,
        ...
        #expect(manager.activeCount == VoiceManager.maxVoices)
        manager.forceActiveEnvelopesForTesting(0.8)

        // 65th note must steal a slot — pool stays capped
        manager.addVoices(
            midiNote: 100,
            velocity: 1.0,
            unisonVoices: 1,
        ...
        #expect(manager.activeCount == VoiceManager.maxVoices)
        #expect(manager.isMIDINoteActive(100))
    }

    @Test func voiceManagerStealReleaseRampsWithOneMsOverride() async throws {
        // ADSR with long preset release; steal override must finish ~1 ms, not hang.
        var env = ADSREnvelope()
        env.release = 0.5
        var phase = EnvelopePhase.sustain
        var time: Double = 0
        var value: Float = 0.8
        let sr = 44_100.0
        let steal = VoiceManager.stealReleaseSeconds

        var finished = false
        // 1 ms @ 44.1k ≈ 44 samples; allow headroom for exp curve
        for _ in 0..<200 {
            value = env.process(
                currentValue: value,
                phase: &phase,
        ...
        // Same envelope without override must still be releasing after 200 samples
        phase = .sustain
        time = 0
        value = 0.8
        for _ in 0..<200 {
        ...
        #expect(phase == .release || phase == .sustain)
        #expect(value > 0.1)
    }

    @Test func audioEngineUsesVoiceManagerCapAndCommandQueue() async throws {
        let engine = AudioEngine()
        var preset = SynthPreset.defaultPreset
        preset.arpMode = .off
        ...
        #expect(engine.voiceManagerForTesting.activeCount == VoiceManager.maxVoices)
        #expect(engine.activeNoteCountForTesting == VoiceManager.maxVoices)
    }
